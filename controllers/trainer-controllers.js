...
const createCoach = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError('Something went wrong creating the coach', 500);
        return next(error);
    }
    ...
    res.status(201).json({message: "Success", coachName:newCoach.coachName, coachID: newCoach.id, token: token });
}
...
const searchCoaches = async (req, res, next) => {
    ...
    res.json({users:foundCoach.map(coach => coach.toObject({getters: true}))})
}
...
const loginCoach = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError('Error with finding coach please try again later',500)
        return next (error)
    }
    ...
    res.json({message: "Coach login successful", coachID: verifiedCoach.id, token:token})
}
